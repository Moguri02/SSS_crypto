...
이 코드는 AES 알고리즘의 구현으로, 특히 ECB, CTR 및 CBC 모드를 포함합니다.
블록 크기는 aes.h에서 선택할 수 있으며, 선택 가능한 옵션은 AES128, AES192, AES256입니다.

이 구현은 다음의 테스트 벡터를 기준으로 검증되었습니다:
  미국 국가 표준 기술 연구소(NIST) 특별 출판물 800-38A 2001년 판
...
#define Nb 4   // Number of columns (32-bit words) in the state. For AES, Nb = 4.
#define Nk 8   // Number of 32-bit words in the key. For AES-256, Nk = 8.
#define Nr 14  // Number of rounds. For AES-256, Nr = 14.
...
void KeyExpansion(uint8_t* round_keys, const uint8_t* Key)
{
    ...
    {
        k = (i - 1) * 4;
        tempa[0] = round_keys[k + 0];
        tempa[1] = round_keys[k + 1];
        tempa[2] = round_keys[k + 2];
        tempa[3] = round_keys[k + 3];
...
void Addround_keys(uint8_t round, state_t* state, const uint8_t* round_keys)
{
    ...
    for (i = 0; i < 4; ++i)
    {
        for (j = 0; j < 4; ++j)
        {
            (*state)[i][j] ^= round_keys[(round * Nb * 4) + (i * Nb) + j];
        }
    }
}
...
void Cipher(state_t* state, const uint8_t* round_keys)
{
    ...
    Addround_keys(0, state, round_keys);
    ...
    for (round = 1; ; ++round)
    {
        ...
        MixColumns(state);
        Addround_keys(round, state, round_keys);
    }
    // Add round key to last round
    Addround_keys(Nr, state, round_keys);
}
...
// InvSubBytes 함수는 상태 행렬의 값을 역 S-box 값으로 대체합니다.
...
// InvShiftRows 함수는 상태 행렬의 행을 오른쪽으로 이동합니다.
...
{
    state_t* state = (state_t*)buf;
    InvCipher(state, ctx->round_keys);
}
...
void encrypt_data(byte* data, size_t size, const byte* key, byte* round_keys)
{
    ...
    {
        AES_ECB_encrypt(&ctx, data + (i * BLOCK_SIZE));
    }
}
